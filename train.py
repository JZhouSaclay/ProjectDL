...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        tokenizer = Tokenizer(WordLevel(unk_token="[UNK]"))  # unknown tokens
        tokenizer.pre_tokenizer = Whitespace()
        # unknown tokens, padding tokens, start of sentence tokens,
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            ...
            porj_output = model.project(decoder_output)  # (B,seq_len,vocab_tgt_len)

            label = batch["label"].to(device)  # (B,seq_len)

            # (B,seq_len,tgt_vocab_size) - > (B*seq_len,tgt_vocab_size)
            loss = loss_fn(
                porj_output.view(-1, tokenizer_tgt.get_vocab_size()), label.view(-1)
            )
